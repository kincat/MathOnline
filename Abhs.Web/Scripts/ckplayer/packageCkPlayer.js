...
!(function() {
	...
	var ckplayer = function (obj) {
	    ...
	    // 新增的属性 视频总时间
	    this.videoTimesCount = 0;

	    // 监听当前的时间点
	    this.videoCurrTimes = 0;

        // 是否禁止倍速改变
	    this.disPlaybackrate = true;

        // 是否正在播放
	    this.videoIsPlay = null;

        // 保存一个播放状态
	    this.lastVideoIsPlay = null;
    ...
    ckplayer.prototype = {
		...
		*/
        getVideo: function() {
            ...
            if (this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
                ...
                this.css(this.V, 'backgroundColor', '#ff');
                //创建一个画布容器
		...
		*/
        timeButtonMouseDown: function() {
            ...
            var timeBoBgmousemove = function (event) {
                ...
                // 设置了打点说明
                if (thisTemp.vars['showPoints'] != undefined && thisTemp.vars['showPoints'] != null) {
                    for (var i = thisTemp.vars['showPoints'].length - 1; i >= 0; i--) {
                        if (showTitle == "" && thisTemp.vars['showPoints'][i].BegTime <= eventTime) {
                            showTitle = thisTemp.vars['showPoints'][i].Text;
                        }
                    }
                }
                ...
                var buttonPromptObj = {
                    ...
                    title: showTitle == "" ? thisTemp.formatTime(eventTime) : showTitle,
                    x: eventX,
                    y: timeBoBgXY['y']
                };
		...
		*/
        controlBarHide: function (hide) {
            ...
            // 禁止自动隐藏
            if (this.vars["isAutoHideBar"] != undefined && this.vars["isAutoHideBar"] == false)
            {
                return;
            }
		...
		*/
        newPlaybackrate: function(title) {
            ...
            // 禁止改变速度
            if (this.disPlaybackrate)return;
		...
		*/
        fullScreen: function () {
            ...
            // 已经全屏 推出
            if ($('.content35 .videobox').hasClass('videoboxbig'))
            {                
                FullClose();
            }
            else // 全屏
...
//均速运动
		...
		*/
        ...
        //  视频播放时时播放后调取的函数
        timeHandlerCallBack: function () {
        },

        // 当播放器加载后执行的函数
        loadedHandlerCallBack: function () {

            var currPlay = this;

            // 获取视频时长
            this.addListener('duration', function (duration) {
                currPlay.videoTimesCount = Math.round(duration);
            });

            //监听播放时间
            this.addListener('time', function (time) {
                currPlay.videoCurrTimes = time;
                currPlay.timeHandlerCallBack();
            });

            //  视频播放时时播放点获取
            this.addListener('play', function () {
                currPlay.videoIsPlay = true;
            }); //监听播放时间
